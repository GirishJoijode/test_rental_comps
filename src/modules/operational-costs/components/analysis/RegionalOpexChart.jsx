...
import { buildRegionalOpex } from '../../utils/analysis'
...
export default function RegionalOpexChart({ records, basis }) {
  const data = useMemo(() => buildRegionalOpex(records, basis), [records, basis])
  const layout = verticalBarLayout(data.length)
  const basisLabel = basis === 'sqft' ? '£ / Sq Ft' : '£ / Flat'

  return (
    <ChartCard
      title="OpEx by Region"
      subtitle={`Average OpEx ${basisLabel} within analysed evidence`}
